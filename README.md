...
さらに **SharePoint Stream** の Teams 会議録画ページからは、字幕トランスクリプトを **VTT ファイルでダウンロード** できます。
...
### VTTダウンロード（SharePoint Stream / Teams 会議録画）

Teams 会議の録画動画ページ（`*.sharepoint.com/.../stream.aspx`）のヘッダーに **「VTTダウンロード」** ボタンが追加されます。クリックで会議トランスクリプト（字幕）を `.vtt` ファイルとして保存できます。トランスクリプトが存在しない録画ではボタンは表示されません。
