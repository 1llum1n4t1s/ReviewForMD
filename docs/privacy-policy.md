...
- Microsoft Teams（teams.microsoft.com / teams.live.com / teams.cloud.microsoft）のチャット/チャネルから、メッセージ履歴（送信者・日時・本文・リアクション・添付）を取得し、Markdown ファイル（添付込みの場合は ZIP ファイル）としてダウンロードする
...
本拡張機能がアクセスするデータ（PR タイトル・本文・レビューコメント、SharePoint Stream の VTT トランスクリプト、および Microsoft Teams のチャットメッセージ・添付ファイル/画像）は以下の方法でのみ処理されます。

- ブラウザ内のメモリ上で Markdown / VTT 形式に変換（整形）されます
- Teams の「添付ごとZIP」では、添付ファイル・画像をユーザー自身の既存ログインセッションを用いて Microsoft のサーバーから取得し、ブラウザ内メモリ上で ZIP アーカイブにまとめます（取得先は Microsoft 関連ドメインに限定）
- ユーザーの明示的な操作（ボタンクリック）に応じて、次のいずれかが行われます
  - クリップボードへのコピー（「MDコピー」ボタン）
  - `.md` / `.vtt` / `.zip` ファイルとしてのダウンロード（「MDでダウンロード」「VTTダウンロード」「添付ごとZIP」ボタン）
- 処理後、メモリ上のデータは破棄されます
...
ただし、ユーザーが「MDでダウンロード」「VTTダウンロード」「添付ごとZIP」ボタンをクリックした場合のみ、ブラウザの標準ダウンロード機能を通じて、ユーザー自身の端末のダウンロードフォルダに `.md` / `.vtt` / `.zip` ファイルが保存されます。これはユーザーの明示的な意思に基づく保存であり、拡張機能側は保存後のファイルにアクセスしません。
