...
- **Teams チャット**: Microsoft Teams のチャット/チャネルを自動スクロールで収集し、Markdown でダウンロード（収集する期間を選べて、進捗を見ながらいつでも中止できます）
...
Microsoft Teams（`teams.microsoft.com` / `teams.live.com` / `teams.cloud.microsoft`）のチャット・チャネルを開いてアイコンをクリックすると、**収集する期間**（過去1か月／2か月／3か月）を選んでから次のボタンを押せます。

- **「MDでダウンロード」** — 選んだ期間までさかのぼってチャットを自動スクロール収集し、送信者・日時・本文・リアクション・添付リンクを 1 つの `.md` ファイルとして保存。
