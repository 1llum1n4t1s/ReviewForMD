...
Microsoft Teams（`teams.microsoft.com` / `teams.live.com` / `teams.cloud.microsoft`）のチャット・チャネルを開いてアイコンをクリックすると、**収集する月**（今月／先月／2か月前／3か月前）を選んでから次のボタンを押せます。例えば 5/10 に「今月」を選ぶと 5/1〜5/10 が対象になります。

- **「MDでダウンロード」** — 選んだ月のチャットを自動スクロール収集し、送信者・日時・本文・リアクション・添付リンクを 1 つの `.md` ファイルとして保存。
